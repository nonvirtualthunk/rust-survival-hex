...
pub struct TacticalGui {
    ...
    unit_icon: Widget,
    character_stat_widgets: Vec<Widget>,
...
impl TacticalGui {
    pub fn new(gui: &mut GUI) -> TacticalGui {
        ...
        let mut prev_widget = None;
        let mut character_stat_widgets = Vec::new();
        for stat in &character_stats {
            let y = match prev_widget {
                Some(wid) => Positioning::DeltaOfWidget(wid, 4.px(), Alignment::Bottom),
                None => Positioning::DeltaOfWidget(unit_icon_background.id, 10.px(), Alignment::Bottom)
            };
            let widget = Widget::text(stat.name, 16)
                .position(Positioning::Constant(2.ux()), y)
                .parent(&main_widget)
                .apply(gui);
            prev_widget = Some(widget.id);

            character_stat_widgets.push(widget);
        }


        let skills = ListWidget::new()
            .parent(&main_widget)
            .with_body(|body: Widget|
                body.size(Sizing::Constant(30.ux()), Sizing::Constant(50.ux()))
                    .position(Positioning::Constant(2.ux()), Positioning::Constant(30.ux()))
            ).apply(gui);


        TacticalGui {
            character_stats,
            character_stat_widgets,
            main_widget,
            name_widget,
            unit_icon,
    ...
    pub fn update_gui(&mut self, world: &mut World, gui: &mut GUI, frame_id: Option<Wid>, game_state: GameState) {
        if let Some(selected) = game_state.selected_character {
            ...
            for i in 0..self.character_stats.len() {
                let stat = &self.character_stats[i];
                let mut widg = &mut self.character_stat_widgets[i];
                ...
                widg.widget_type.set_text(text);
                widg.reapply(gui);
            }

            let mut character_skills: Vec<(Skill, &u32)> = skills.skills.iter().collect();
            character_skills.sort_by_key(|&(_, lvl)| lvl);

            self.skills.update(gui, character_skills.as_ref(), |gui : &mut GUI, parent: &Widget, skill_w: &mut SkillWidget, skill: &(Skill, &u32)| {
                let (skill, lvl): (Skill, &u32) = *skill;
                let skill_info = skill_info(skill);
                let text = format!("{} : {}", skill_info.name, lvl);
                skill_w.text.set_parent(parent).widget_type.set_text(text);
                skill_w.text.reapply(gui);
            }).reapply(gui);
        }
...
impl WidgetDemo {
    pub fn new(gui: &mut GUI) -> WidgetDemo {
        ...
        let sub_window = Widget::new(WidgetType::image("entities/default/defaultium"))
        ...
        let left_sub_window = Widget::window(Color::white(), 1)p
        ...
        let button = Button::new("Test Button")
            ...
            .parent(&test_window)
            .with_on_click_2(|tactical: &mut TacticalMode, world: &mut World| {
                info!("Button click from callback detected");
            })
            .apply(gui);
    ...
    pub fn update(&mut self, gui: &mut GUI) {
        if self.button.clicked(gui) {
            info!("Sync processing of button detected");
        }
    }
}
