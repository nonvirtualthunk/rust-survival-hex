            ...
            if let UIEvent::WidgetEvent(wevent) = event {
                if let WidgetEvent::ListItemClicked(index) = wevent {
                    if let Some(attack_ref) = AttackReference::of_attack(view, character, &attacks[*index]).as_option() {
                        control.event_bus.push_event(ControlEvents::AttackSelected(attack_ref.clone()));
                    } else {
...
impl AttackDetailsWidget {
    ...
    pub fn new() -> AttackDetailsWidget {
        ...
        let to_hit_div = Widget::div().below(&name, 1.px()).parent(&body);
        let damage_div = Widget::div().below(&name, 1.px()).right_of(&to_hit_div, 9.px()).parent(&body);

        let to_hit = Widget::text("to hit", 14).parent(&to_hit_div);
        ...
        let to_hit_details_div = Widget::div().below(&to_hit, 10.px()).parent(&to_hit_div);

        let to_hit_details = Widget::text("to hit details", 12).color(positive_color).parent(&to_hit_details_div);
        let to_miss_details = Widget::text("to miss details", 12).below(&to_hit_details, 2.px()).color(negative_color).parent(&to_hit_details_div);


        let damage_details_div = Widget::div().match_y_of(&to_hit_details_div).parent(&damage_div);
        let damage_dice_details = Widget::text("damage dice details", 12).color(neutral_color).parent(&damage_details_div);
        let damage_bonus_details = Widget::text("damage bonus details", 12).below(&damage_dice_details, 1.px()).color(positive_color).parent(&damage_details_div);
        let damage_absorption_details = Widget::text("damage absorption", 12).below(&damage_bonus_details, 1.px()).color(negative_color).parent(&damage_details_div);
