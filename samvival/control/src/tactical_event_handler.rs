...
pub fn animation_elements_for_new_event(world_view: &WorldView, wrapper: &GameEventWrapper<GameEvent>) -> Vec<Box<AnimationElement>> {
...
fn animate_attack(world_view: &WorldView, attacker: Entity, defender: Entity, damage_done: u32, hit: bool) -> Vec<Box<AnimationElement>> {
    ...
    let blocking_duration = if hit { base_duration * 0.5 } else { base_duration };
    ...
    let miss_anim_start_point = swing_at_enemy.blocking_duration() * 0.5;
